...
export const MeasuringSeries = props => {
    const { video, onSeriesMeasured, amount = 5, mayStart = true, type = "" } = props;
            ...
            <th>
                {videos.map((currentVideo, i) => (
                    <VideoLoading
                        ...
                        mayStart={measuredVideos.length >= i}
                        loadingFinished={video => {
                            if(!video.succesfullyFinished) {
                                console.log("asdasd", measuredVideos.length)
                            }
                            setMeasuredVideos([
                                ...measuredVideos,
                                {
